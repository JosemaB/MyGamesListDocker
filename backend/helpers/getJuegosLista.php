...
try {
    ...
    if ($datos) {
        ...
        $sql = "SELECT id_juego, titulo, image, link_juego FROM lista_videojuegos WHERE id_lista = ?";

        // Preparar la consulta
        $stmt = $conexion->prepare($sql);
        $stmt->bind_param("i", $idLista); // "i" significa que el parámetro es un entero
...
if (isset($error)) {
    ...
    echo json_encode(["success" => true, "juegos" => $listaJuegos]);
}
exit();